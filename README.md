...
### Step 1: Coordinates system, Field of View and PathFinding
...
![Path Preview](docs/images/pathPreview.png)

**Field of View** determines which hexes are visible around the unit. For each hex in range, a raycast checks for obstacles along the line of sight. The algorithm runs in O(n) per FOV update, which is simple and efficient enough since I avoid recomputing it on every update.
<div align="center">
...
I wanted to use an event-driven architecture, but having every component interaction go through the event bus would create too much noise. So I use a hybrid approach: components that work closely together depend on each other directly via constructor injection, while the event bus handles higher-level coordination like user input, server updates, and game actions (move, attack, skip turn, forfeit).
...
The field of view uses a precomputed visibility map (O(n² × r) at game start) to avoid running the O(n) raycast algorithm on every FOV update. This map only gets recomputed when the playable area shrinks, since that's the only moment where obstacles can disappear and affect visibility.
...
- Map creation and unit spawning are currently done on both the frontend and backend. Only the backend should handle this and send the result to clients.
